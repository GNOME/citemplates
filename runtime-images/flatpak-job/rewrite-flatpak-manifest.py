    ...
    for i in range(mods.get_length()):
        ...
        if mod.get_string_member("name") == args.module_name:
            ...
            if sources.get_object_element(0).get_string_member("type") == "git":
                ...
                new_sources.add_element(
                    Json.from_string(f'{{"type": "dir", "path": "{path}"}}')
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Prepare a flatpak manifest for CI execution"
    )
    parser.add_argument(
        "flatpak_manifest", help="filename of the flatpak manifest (JSON or YAML)"
    )
    parser.add_argument("module_name", help="name of the main module in the manifest")
    parser.add_argument(
        "config_opt",
        nargs="*",
        help="replacement values for config-opts of main module",
    )
    parser.add_argument(
        "--run-tests",
        action=argparse.BooleanOptionalAction,
        help="replace run-tests value of main module",
    )
